    ...
    if (playerId == 0) {
      if (areaOccupation[areaNum] == "g" && playerTurn == 0) {
        ...
        if (playerOneFunds - cost < 0) {
          msg = "You don't have enough funds or it's the other players turn !";
        ...
        msg = "You don't own this area or it's the other players turn !";
        socket.emit("message", msg);
      }
    } else {
      ...
      if (areaOccupation[areaNum] == "g" && playerTurn == 0) {
        if (playerOneFunds - cost < 0) {
          ...
          if (playerOneBases[areaNum] == 1) {
            msg = "There is already a base on this area or it's the other players turn !";
            socket.emit("message", msg);
          } else {
            playerOneFunds = playerOneFunds - cost;
      ...
      if (areaOccupation[areaNum] == "r" && playerTurn == 1) {
        if (playerTwoFunds - cost < 0) {
          ...
          if (playerTwoBases[areaNum] == 1) {
            msg = "There is already a base on this are or it's the other players turn !";
            socket.emit("message", msg);
          } else {
            playerTwoFunds = playerTwoFunds - cost;
  ...
  socket.on("show-available", (playerId, areaNum) => {
    if (playerId == 0) {
      if (areaOccupation[areaNum] != "g" || playerTurn == 1) {
        msg = "You can't move units from an area you don't own or it's the other players turn !";
        socket.emit("message", msg);
      } else {
  ...
  socket.on("move-units", (playerId, fromNum, toNum, movingAmount) => {
    ...
    if (playerId == 0) {
      if (movingAmount > playerOneUnits[fromNum]) {
        msg = "You don't have enough units to move or it's the other players turn !";
        socket.emit("message", msg);
      } else if (movingAmount == 0) {
        msg = "You want to move zero units or it's the other players turn !";
        socket.emit("message", msg);
      } else {
        //if its ours
        if (areaOccupation[toNum] == "g" && playerTurn == 0) {
          ...
          io.emit(
            ...
            fromNum,
            toNum,
            playerOneUnits[fromNum],
            playerOneUnits[toNum],
            playerTwoUnits[fromNum],
            playerTwoUnits[toNum],
            action,
            hasBase,
            unitSituation
          );
...
function checkOperations(numOfOperations, turnNum) {
  if (numOfOperations == 0) {
    ...
    for (var i = 0; i < 50; i++) {
      if (areaOccupation[i] == "g") {
        playerOneIncome += 25;
      }
      if (areaOccupation[i] == "r") {
        playerTwoIncome += 25;
      }
    }
